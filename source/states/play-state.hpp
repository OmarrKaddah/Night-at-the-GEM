           ...
           // Update Zombie AI
                ...
                for (auto entity : world.getEntities())
                {
                    if (entity->name.find("Zombie") != std::string::npos)
                    {
                        ...
                        if (dist < 1.5f)
                        {
                            ...
                            auto &state = zombieSystem.states[entity];
                            if (state.attackSoundCooldown <= 0.0f)
                            {
                                SOUND_MANAGER->playSound("zombie_attack");
                                state.attackSoundCooldown = 1.5f; // attack sound cooldown
                            }
            ...
            if (gameplayActive && !isDead)
            {
                ...
                if (displayHour != lastHourChime)
                {
                    ...
                    if (displayHour != 12)
                    { // Don't flash 12 immediately at reset if we just flashed 6
            ...
            // --- Render UI (Health + Crosshair) ---
            {
                ...
                if (outcome == GameOutcome::Win)
                {
                    ...
                    if (clockFlashTimer > 0.0f)
                    {
                        ...
                        if (clockMaterial->texture)
                        {
                            ...
                            float alpha = 1.0f;
                            if (clockFlashTimer < 1.0f)
                                alpha = clockFlashTimer;
                ...
                else if (clockFlashTimer > 0.0f)
                {
                    ...
                    if (isNightIntro)
                    {
                        ...
                        if (clockFlashTimer > 7.0f)
                            alpha = 8.0f - clockFlashTimer;
                        else if (clockFlashTimer < 1.0f)
                            alpha = clockFlashTimer;
                        else
                            alpha = 1.0f;
                    ...
                    else
                    {
                        ...
                        if (clockFlashTimer < 1.0f)
                            alpha = clockFlashTimer; // Fade out last second
