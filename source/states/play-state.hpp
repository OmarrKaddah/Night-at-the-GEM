...
namespace our
{
    class Playstate : public State
    {
            ...
            if (config.contains("navigation"))
            {
                ...
                if (navConfig.contains("floor0"))
                {
                    ...
                    if (f0.contains("obstacles"))
                    {
                        ...
                        {
                            glm::ivec2 obsMin(obs["min"][0], obs["min"][1]);
                            glm::ivec2 obsMax(obs["max"][0], obs["max"][1]);
                            floor0Grid.setObstacle(obsMin, obsMax);
                        }
                    }
                }
                ...
                if (navConfig.contains("floor1"))
                {
                    ...
                    if (f1.contains("obstacles"))
                    {
                        for (auto &obs : f1["obstacles"])
                        {
                            glm::ivec2 obsMin(obs["min"][0], obs["min"][1]);
                            glm::ivec2 obsMax(obs["max"][0], obs["max"][1]);
                            floor1Grid.setObstacle(obsMin, obsMax);
                        }
                    }
                }
                ...
                if (navConfig.contains("floor2"))
                {
                    ...
                    if (f2.contains("obstacles"))
                    {
                        for (auto &obs : f2["obstacles"])
                        {
                            glm::ivec2 obsMin(obs["min"][0], obs["min"][1]);
                            glm::ivec2 obsMax(obs["max"][0], obs["max"][1]);
                            floor2Grid.setObstacle(obsMin, obsMax);
                        }
                    }
                }
        ...
        void onImmediateGui() override
        {
            ...
            {
                char line[128];
                sprintf(line, "Rounds Survived: %d", roundsSurvivedAtEnd);
                centeredText(displaySize.y * 0.42f, 1.6f, line);

                sprintf(line, "Kills: %d", killsAtEnd);
        ...
        void onDestroy() override
        {
            ...
            std::cout << "Playstate::onDestroy - Stopping Sounds" << std::endl;
