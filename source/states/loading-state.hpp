    ...
    void onInitialize() override
    {
        ...
        if (config.contains("assets"))
        {
            ...
            if (assets.contains("shaders"))
                queueAssets(assets["shaders"], [](const auto &j)
                            { our::AssetLoader<our::ShaderProgram>::deserialize(j); });

            if (assets.contains("textures"))
                queueAssets(assets["textures"], [](const auto &j)
                            { our::AssetLoader<our::Texture2D>::deserialize(j); });

            if (assets.contains("samplers"))
                queueAssets(assets["samplers"], [](const auto &j)
                            { our::AssetLoader<our::Sampler>::deserialize(j); });

            if (assets.contains("meshes"))
                queueAssets(assets["meshes"], [](const auto &j)
                            { our::AssetLoader<our::Mesh>::deserialize(j); });

            if (assets.contains("materials"))
                queueAssets(assets["materials"], [](const auto &j)
                            { our::AssetLoader<our::Material>::deserialize(j); });
        }
...
void onImmediateGui() override
{
    ...
    int dotCount = ((int)(time * 2.0f)) % 4;
    std::string dots(dotCount, '.');
    std::string loadingText = "LOADING" + dots;

    ImGui::SetNextWindowPos(ImVec2(x, y - 60.0f));
    ...
    ImGui::Begin("LoadingText", nullptr, flags);
    ...
    draw->AddText(ImVec2(cursor.x - 1, cursor.y), black, loadingText.c_str());
    draw->AddText(ImVec2(cursor.x + 1, cursor.y), black, loadingText.c_str());
    draw->AddText(ImVec2(cursor.x, cursor.y - 1), black, loadingText.c_str());
    draw->AddText(ImVec2(cursor.x, cursor.y + 1), black, loadingText.c_str());
    draw->AddText(cursor, black, loadingText.c_str());

    ImGui::Dummy(ImGui::CalcTextSize(loadingText.c_str()));
    ImGui::End();
}
    ...
    void onDraw(double deltaTime) override
    {
        ...
        if (!barMaterial)
        {
            ...
            // Use menu highlight color (Pale Yellow/Cream)
            barMaterial->tint = glm::vec4(1.0f, 0.92f, 0.6f, 1.0f);
        ...
        // --- Draw BLACK BACKGROUND FRAME for loading bar ---
        static our::TintedMaterial *barBgMaterial = nullptr;
        if (!barBgMaterial)
        {
            barBgMaterial = new our::TintedMaterial();
            barBgMaterial->shader = new our::ShaderProgram();
            barBgMaterial->shader->attach("assets/shaders/tinted.vert", GL_VERTEX_SHADER);
            barBgMaterial->shader->attach("assets/shaders/tinted.frag", GL_FRAGMENT_SHADER);
            barBgMaterial->shader->link();

            barBgMaterial->tint = glm::vec4(0, 0, 0, 0.85f); // Black
            barBgMaterial->pipelineState.blending.enabled = true;
            barBgMaterial->pipelineState.blending.sourceFactor = GL_SRC_ALPHA;
            barBgMaterial->pipelineState.blending.destinationFactor = GL_ONE_MINUS_SRC_ALPHA;
        }

        // Slight padding around bar
        float pad = 6.0f;

        glm::mat4 bgM =
            glm::translate(glm::mat4(1.0f), glm::vec3(x - pad, y - pad, 0.0f)) *
            glm::scale(glm::mat4(1.0f),
                       glm::vec3((bw * size.x) + pad * 2.0f, h + pad * 2.0f, 1.0f));

        barBgMaterial->setup();
        barBgMaterial->shader->set("transform", VP * bgM);
        bar->draw();
