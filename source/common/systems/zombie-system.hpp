...
namespace our
{
    ...
    public:
        struct ZombieState
        {
            ...
            float attackCooldown = 0.0f;      // controls attack timing
            float attackSoundCooldown = 0.0f; // prevents sound spam
        };
        ...
        {
            if (y < 1.5f)
                return 0; // Ground floor + stairs (Y around -0.4 to 1.5)
            else if (y < 4.0f)
                return 1; // First floor (Y around 1.5 to 4.0)
            else
                return 2; // Second floor (Y >= 4.0)
        }
        ...
        void update(World *world, float deltaTime)
        {
            ...
            counter++;
            std::cout << "Zombie update frame: " << counter << std::endl;
